...
import { MapPin, Image, Pencil, Trash2 } from 'lucide-react';
import type { Destination } from '../../api/destination';
import { useAuthStore } from '../../stores/authStore';

interface Props {
    destination: Destination;
    onEdit: (dest: Destination) => void;   // new props
    onDelete: (dest: Destination) => void;
}

export default function DestinationCard({ destination, onEdit, onDelete }: Props) {
    const { user } = useAuthStore();
    const isOwner = user?.id === destination.userId;  // compare userId

    return (
        <div className="relative group bg-white rounded-lg shadow hover:shadow-md transition overflow-hidden">
            <Link to={`/destination/${destination.id}`} className="block">
                <div className="relative h-48 bg-gray-200">
                    {destination.photos.length > 0 ? (
                        <img src={destination.photos[0].url} alt={destination.name}
